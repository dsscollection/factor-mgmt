...
% XX NH I don't know what is going on here. What was your point in the sentence? 
...
The unfortunate behavior of factors in \proglang{R} has led to an online movement against the default behavior of many data import functions to take any variable composed as strings and automatically convert the variable to a factor. The tidyverse is part of this movement, with functions from the \pkg{readr} package defaulting to leaving strings as-is. (Others have chosen to add \verb#options(stringAsFactors=FALSE)# into their startup commands.)
...
\subsection*{Robust method}
% XX NH this isn't exactly the starting point here. 
<<>>=
GSS <- GSS %>%
