...
For example, many analysts use testing and training data when working with models. Without careful thought toward levels of categorical variables, there can be a mismatch between the levels present in the training data and those present in the testing data. If a particular  level was not present in the training data, the model will not be able to make predictions for the observations in the testing data with that level. Even worse, if the two sets have the same number of levels, the model may produce predictions by matching the order of the levels rather than the labels. 
...
Thanks to Hadley Wickham, Mine \c{C}etinkaya-Rundel, Johanna Hardin, Zev Ross, and Colin Rundel for helpful comments and suggestions on an earlier draft.
