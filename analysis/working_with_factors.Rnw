...
Another \pkg{base} approach that will not suffer from spelling mistakes is to use numeric indexing to reorder the levels. Again, we need to make sure we're using the indexing within a \verb#factor()# call. 
...
Notice the levels we did not mention end up at the back end of the ordering. 
...
Note that you need to be very sure that the strings including a number have a relevant number. If one of the levels was labeled `2 or more people in household` it would be converted to the number 2. This would accidentally add a number that is not meaningful. 
