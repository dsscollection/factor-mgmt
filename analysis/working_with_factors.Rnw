...
To do this, we will consider data from the General Social Survey. There are some import issues inherent to the data which are not particular to categorical data, so that processing is in Appendices \ref{load, rename}. We'll work with the data that has cleaned variable names.
...
\subsection*{Defensive coding}

In addition to using tidy tools to recode factor levels, it is good to practice defensive coding. 
...
\subsection{Creating derived categorical variable}
...
\subsection{Creating derived categorical variables}
...
\appendix
\section*{Loading the data}\ref{load}
...
\section*{Renaming the variables}\ref{reload}
