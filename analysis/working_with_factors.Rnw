...
We also might want to ensure the factor labels are exactly what we were expecting. Perhaps we want to make sure our Sex variable has two categories, with particular levels. We can use \verb#expect_equivalent()# from the \pkg{testthat} package to make this check. 
