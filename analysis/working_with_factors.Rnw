...
<<>>=
summary(GSS$PoliticalParty)
GSS$NewParty <- as.character(GSS$PoliticalParty)
GSS$NewParty[GSS$PoliticalParty == "Ind,near dem"] <- 
  "Independent, near democrat"
GSS$NewParty[GSS$PoliticalParty == "Ind,near rep"] <- 
  "Independent, near republican"
GSS$NewParty[GSS$PoliticalParty == "Not str democrat"] <- 
  "Not strong democrat"
...
levels(GSS$OpinionOfIncome)
levels(GSS$OpinionOfIncome) <- 
  levels(GSS$OpinionOfIncome)[c(5,1:3,6,4,7)]
levels(GSS$OpinionOfIncome)
@
...
The \verb#recode()# function does what we want, this time for an example using race/ethnicity.
<<>>=
levels(GSS$Race)
GSS <- GSS %>% 
...
library(readr)
GSS <- GSS %>%
  mutate(dplyrAge = parse_number(Age)) %>%
  mutate(dplyrAge = ifelse(dplyrAge < 65, "18-65", "65 and up"),
...
expect_equivalent(levels(GSS$Race), c("Nonwhite", "White")) 
@
...
Categorical variables arise commonly in most datasets.  
Aspects of data wrangling in R involving categorical variables can be problematic and error-prone.
