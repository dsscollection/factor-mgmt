...
OUR RESPONSE: Please note that we're now running the development version of dplyr that has the fix for the problem with `case_when()`. We added spaces around all double equals in the paper. We've changed "this can't be NA" to "can't put NA in place of "missing"" Is it possible to set things to missing in `case_when()`? We don't see a way to do that. When we try, we get the error message `RHS of case 5 (NA): must be type character, not logical`. 
XX NH re: tally()
